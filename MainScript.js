function HGExecuteTask(taskCode){
	eval(`;(function(){
		...
		function HGShowFeedback(){
		  var inputs = document.querySelectorAll("[HGInput]") // get all inputs
		  for (var InpNum = 0; InpNum < inputs.length; InpNum++){
			if((inputs[InpNum].getAttribute(HGNumber) == HGInstance) && (typeof HGFeedbackMap[inputs[InpNum].getAttribute("HGInput")] != "undefined")){ // if feedback exists, set content to feedback
